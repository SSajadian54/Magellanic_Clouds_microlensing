...
const double Rjupiter = 71.492*pow(10.,6.0);//radius of jupiter in [m]
...
const double epss=0.0000000000038654723954932645; 
...
const double sigma[M]={0.022, 0.022, 0.02, 0.025, 0.025};
...
struct dete{
    double lmass[Nm], dmass;
    double tE[2][Nm], RE[2][Nm], rho[2][Nm], opt[2][Nm];
    double Dl[2][Nm], nsim[2][Nm], ndet[2][Nm];    
    double Ds[2][Nm], rhol[2][Nm], Ep[2][Nm];
    double oplb1[2], oplb2[2], gama[2], Nevent[2];    
    double telb[2], mlb[2], nlb[2],  efte[2]; 
};
struct CMD{
...
///****************************************************************************
   ...
   fil1=fopen("./files/pars_1.txt","w");
   fil2=fopen("./files/pars_2.txt","w");
   fclose(fil1); 
   fclose(fil2); 
...
///&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
  /* FILE*  mapd; 
   mapd=fopen("./files/Map_density_SMC/mapd_smc.txt","w"); 
   for(int l1=0; l1<nn;      l1+=1){
   for(int l2=0; l2<int(nn); l2+=1){
   s.right = double(RaSMC+  (l1-nn*0.5)*ddeg); ///degree
   s.decli = double(DecSMC+ (l2-nn*0.5)*ddeg); ///degree
   //===========================================================
   ...
   s.FI=  s.bg/RA;
   Disk_model(s, l1, l2);
   fprintf(mapd,"%.5lf  %.5lf    %.5lf  %.5lf   %.5lf  %.5lf    %.5lf  %.5lf  %.5lf   %.10lf  %.10lf  %.6lf\n",
   s.lg,s.bg,s.right, s.decli,double(s.right-RaSMC),double(s.decli-DecSMC),s.xv,s.yv,s.zv,log10(s.Rostart),log10(s.Nstart),s.Avv);}
   cout<<"l1:  "<<l1<<"\t lat:  "<<s.bg<<endl; }
   exit(0);  */
///&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&    
    double Nmw=0.0, Nsmc=0.0;
    int Nde, l1a, l2a, Nsi=0, hh;
    double magn, Magn, deno,magni, magni0; 
    double  dist, mindd;
    char filname1[40], filname2[40];
    double mdis, disex; 
    double flagb, maga[M], flagdd[M], sum; 
  ...
  // cout<<"*********************** mstep:   "<<mstep<<endl;
   ...
   Nde=0;
   do{
   s.right= double((double)rand()/(double)(RAND_MAX+1.))*4.0 + 11.0;//[11,  15]
   s.decli= double((double)rand()/(double)(RAND_MAX+1.))*7.0 - 73.0;//[-73,-66]
   l1a=int( double(s.right-RaSMC )/ddeg + nn*0.5); 
   l2a=int( double(s.decli-DecSMC)/ddeg + nn*0.5); 
   if(l1a>=nn)  l1a=nn-1;
   if(l2a>=nn)  l2a=nn-1; 
   if(l1a<1)    l1a=1;
   if(l2a<1)    l2a=1;
   ...
   // flagb= double((double)rand()/(double)(RAND_MAX+1.));
    ...
    if(sum>1.0){
    fil1=fopen("./files/pars_1.txt","a+");
    fil2=fopen("./files/pars_2.txt","a+");
    ...
    log10(l.tE), log10(l.RE/AU), l.mul, l.Vt, log10(fabs(s.u0)), l.t0,fl.finiteL,Nmw, Nsmc, s.right, s.decli, mstep);//44
    for(int i=0;i<M; ++i)
    ...
    cout<<"Nmw:  "<<Nmw<<"\t Nsmc:  "<<Nsmc<<"\t Nde:  "<<Nde<<"\t Nsim:  "<<Nsi<<endl;
    cout<<"==============================================================="<<endl;}
    Nsi+=1;
    }while(Nde<200000);//d.nlb[0]<30 or d.nlb[1]<30); 
    //}
   return(0);
}
////==========================================================================
...
void func_lens(lens & l, source & s, int mstep)
{
   ...
   double  randflag=((double)rand()/(double)(RAND_MAX+1.))*s.Rostar0[l.numl];
    ...
    if(l.Ml>=5.2 and l.Ml<=thre)  f=6.667*pow(l.Ml, -1.73);
    }while(test>f);  
    l.Ml= double(l.Ml*Mearth/Msun); 
     ...
     //double mmin=-11.0;//
     //double mmax=log10(15.0*Mjupiter/Msun);//log10(0.001);///brown dwarfs
   ...
   // l.Ml= double(min + RandN(min*0.025,4.0) );//in Sun mass
    ...
    //if(l.dt<cade){ cout<<"ERROR dt:  "<<l.dt<<"\t cade:  "<<cade<<endl; int uue;  cin>>uue; }
