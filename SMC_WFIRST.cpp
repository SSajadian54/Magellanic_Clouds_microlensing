...
const double MJS= double(Mjupiter/Msun);
const double RJS= double(Rjupiter/Rsun); 
const double epss=0.0000000038654723954932645; 
const double Tobs= 72.0; 
const double season=72.0;
const double Mearth= 5.972*pow(10.0,24.0);// kg
const int nte=70;//// number of tE range 
const int nrd=10000; 
...
const double bSMC= -45.9564775;
const double lSMC= 302.7800334; 
const double RaSMC= 13.187;
const double DecSMC=-71.171;  
const double sSMC= 5.0;///degree (half of size of SMC)
const double DSMC= 61.7;///KPC 
const double ddeg=0.15; 
const int nn=int(100);
const int NExt=int(136); 
...
struct dete{
    double tE[2], RE[2], rho[2], opt[2];
    double Dl[2], nsim[2], ndet[2];    
    double Ds[2], rhol[2], Ep[2];
    double oplb1[2], oplb2[2], gama[2], Nevent[2], Nevent2[2];    
    double efte[2]; 
    double ML[2]; 
    double efii[2]; 
};
...
///****************************************************************************
   ...
   FILE* convert; 
   convert=fopen("./files/convert_coordinate_3.dat","r");
   int coo=0; 
   for(int i=0; i<100; ++i){
   for(int j=0; j<100; ++j){
   ...
   cout<<"************** "<<"\t ro_max:  "<<s.Romax<<endl;
    ...
    double magn, maga, Magn, deno,magni, magni0, Astar, Astar0, chi0, chi1, ptt, tt, dchi, effe; 
    double timp, tim,  u, deltaA, dist, mindd;
    int flag_det, flag_peak, Nmicro, flag_ml, nd, Nml=0, Nsi=0, w;
    double magw[Nw], errw[Nw], Nst;
    double Nml_tot[2]={0.0}; 
    double Nstlb[Num]={0.0}, Nsilb[Num]={0.0};
    int fla, flagf, hh; 
    FILE* tempo;    FILE* magg;     FILE* data2;
    char filname1[40], filname2[40];
    double t_0=0.0, t_1=0.0, bbv; 
    double mdis, disex, slope; 


///===========================================================    
    FILE * extinc;  
    extinc=fopen("./files/Extinction_SMC.dat","r");//http://cdsarc.u-strasbg.fr/ftp/J/A+A/628/A51/table3.dat
    int row;  
    double xx, yy, EBV; 
    double Rex[NExt], Dex[NExt], EVI[NExt] ,err[NExt]; 
    for(int i=0; i<NExt; ++i){
    fscanf(extinc,"%d   %lf  %lf %lf  %lf  %lf  %lf  %lf\n",&row,&xx,&yy,&Rex[i],&Dex[i],&EVI[i],&EBV,&err[i]);
    EVI[i]= fabs(EVI[i]);  err[i]=fabs(err[i]); }
    fclose(extinc);    
    

    double avenn=0.0, aveop=0.0; 
  ...
  ///===========================================================   
    FILE * wfirst;  
    wfirst=fopen("./files/sigma_WFIRST.txt","r");
    for(int i=0; i<Nw;++i){
    fscanf(wfirst,"%lf  %lf\n",&magw[i],&errw[i]);}
    fclose(wfirst);    
...
///===========================================================  
    double lte[nte]={0.0};  double eff[nte]={0.0};   
    FILE* effitE; 
    effitE=fopen("./files/effiSMC.dat","r");
    for(int i=0; i<nte;++i){
    fscanf(effitE,"%lf     %lf\n",&lte[i],&eff[i]);
    if(lte[i]<0.0 or eff[i]>1.0008675340 or eff[i]<1.0e-7  or (i>0  and lte[i]<lte[i-1]) ){
    cout<<"ERROR lte[i]:  "<<lte[i]<<"\t eff:  "<<eff[i]<<endl; int iis;  cin>>iis;}}
    fclose(effitE);   
    cout<<" effiSMC.txt  was read !!!  "<<endl;


    
   
    ///=========================================================== 
    FILE* fil1;   FILE* fil2;  FILE* fil5;    FILE* fil6;  
    vbb.LoadESPLTable("./files/ESPL.tbl");
    //fil1=fopen("./files/pmls_1.txt","w");
    //fil2=fopen("./files/pmls_2.txt","w");
    fil5=fopen("./files/tab5_paper.txt","w");
    fil6=fopen("./files/tab5_paperb.txt","w");
    fclose(fil5);  fclose(fil6); 
    //fclose(fil1); 
    //fclose(fil2); 
     
   int l1a, l2a;  
   double nnsi[2], nnde[2];
   double te, re, rho, dl, ds, rhol, efte, op1, op2, EF, gam, Nev, Nev2, ml, Efii, nsi, nde,  opp, nnst, flagb;  
   double Nmw=0.0, Nsmc=0.0, mtss=0.0, wei; 
   for(int mstep=0; mstep<85;  mstep+=10){ 
   mtss+=1.0;  Nml=0;
   cout<<"*********************** mstep:   "<<mstep<<endl;
    ...
    for(int j=0; j<2; ++j){
    d.Ep[j]= 0.00;
    d.nsim[j]=nnsi[j]=epss; 
    d.ndet[j]=nnde[j]=0.0; 
    d.tE[j]=d.RE[j]=d.rho[j]=d.opt[j]=0.0;
    d.ML[j]=0.0; 
    d.efii[j]=0.0; 
    d.Dl[j]=d.Ds[j]=d.rhol[j]=0.0;
    d.gama[j]= d.Nevent[j]=d.Nevent2[j]=0.0;
    d.oplb1[j]=d.oplb2[j]=0.0;
    d.efte[j]=0.0;}
    for(int i=0;i<Num; ++i){Nstlb[i]=Nsilb[i]=0.0;}
    s.nstart=0.0; 
    double Ndte[2][nte]={0.0}; 
    double Nste[2][nte]={0.0};
    Nmw=0.0, Nsmc=0.0;
...
//===================== Monte Carlo Simulation ================================
   do{
   s.right= (double)rand()/(double)(RAND_MAX+1.)*1.4 + RaSMC;
   s.decli= (double)rand()/(double)(RAND_MAX+1.)*1.4 + DecSMC;
   l1a=int( double(-RaSMC +s.right)/ddeg +nn*0.5 + 1.0); 
   l2a=int( double(-DecSMC+s.decli)/ddeg +nn*0.5 + 1.0); 
  
    
    mdis=1000000.0; hh=-1; 
    for(int i=0; i<NExt; ++i){
    disex= (Rex[i]-s.right)*(Rex[i]-s.right) + (Dex[i]-s.decli)*(Dex[i]-s.decli); 
    disex=sqrt(disex); 
    if(disex<mdis) {mdis=disex;  hh=int(i);} } 
    s.Avv= double(3.24*EVI[hh]/1.4)+ ( 1.0*(double)rand()/(double)(RAND_MAX+1.)-0.5)*err[hh];
    if(s.Avv<0.0) s.Avv=0.0; 
    if(hh<0  or s.Avv<0.0 or EVI[hh]<0.0 or err[hh]<0.0){
    cout<<"ERROR hh:  "<<hh<<"\t Avv:  "<<s.Avv<<"\t EVI:  "<<EVI[hh]<<"\t err:  "<<err[hh]<<endl; int uue;  cin>>uue; } 
   ...
   l.Nlens=20000;///int((l.pt2-l.pt1)/l.dt)+2; 
   func_source(s, cm);
   func_lens(l,s, mstep);
   optical_depth(s);
   if(l.dt>cade){
   l.dt=cade*0.2; 
   l.Nlens=int((l.pt2-l.pt1)/l.dt)+2; }
    
   wei=double(s.blend[4]);
   if(l.struc<4) fla=0; ///milky way
   else          fla=1; 
   Nsilb[s.nums]+=1.0*wei; 
...
///==================================================================================================
   ...
   maga=double(s.magb[4] -2.5*log10(s.blend[4]* Magn + 1.0- s.blend[4]) );     
   
   
   flagb= double((double)rand()/(double)(RAND_MAX+1.));
   if(s.magb[4]<=detect[4]){
   Nst+=1.0*wei; 
   Nstlb[s.nums]+=1.0*wei;
    
   /*flagf=0;
   if(l.ro_lens>0.2){  
   flagf=1;
   sprintf(filname1,"./files/MONT/%c%c%c%c%d.dat",'L','I','t','-',Nsi);
   magg=fopen(filname1,"w");
   sprintf(filname2,"./files/MONT/%c%c%c%c%d.dat",'D','A','t','-',Nsi);
   data2=fopen(filname2,"w");
   if(!data2 or !magg){cout<<"cannot make files "<<Nsi<<endl; exit(0);}  }*/
     
   chi0=chi1=0.0; nd=0;  timp=0.0;
   flag_det=flag_peak=flag_ml=0;  
   t_0=t_1=0.0; 
   int *flag=new int[l.Nlens];
   for(int il=0; il<l.Nlens; ++il){
   flag[il]=0;
   tim= double( l.pt1 + l.dt*il );   
   timp += double(l.dt);  
   ptt= double(tim-l.t0); 
   u=sqrt( ptt*ptt/(l.tE*l.tE) + s.u0*s.u0 ); 
   magni0= vbb.ESPLMag2(u, s.ro_star);
   if(magni0<0.999){
   vbb.Tol=1.e-2;
   magni0= vbb.ESPLMag2(u, s.ro_star);
   vbb.Tol=1.e-3; }
   if(l.ro_lens<0.1)  magn= magni0;
   else magn= finitelens(fl, u, s.ro_star, magni0, l.ro_lens);
   Astar0=double(s.blend[4]*magn + 1.0 - s.blend[4] ); 
   magni0=s.magb[4]-2.5*log10(Astar0);///W149
 //  if(flagf>0) fprintf(magg,"%.7lf  %.7lf  %.7lf  %.7lf\n",ptt*24.0, magn, Astar0, magni0);
   if((timp>cade or timp==cade) and  magni0>=satu[4] and  magni0<=detect[4] and tim>0.0  and tim<=72.0){
   w=-1;
   if(magni0<magw[0] or magni0==magw[0]) w=0;
   else if(magni0>magw[Nw-1] or magni0==magw[Nw-1]) w=Nw-1;
   else{
   for(int i=1; i<Nw; ++i){
   if(double((magni0-magw[i])*(magni0-magw[i-1]))<0.0 or  magni0==magw[i-1]){ w=i-1; break; }}}
   if(w==-1){cout<<"mapt[0]: "<<magni0<<"\t w: "<<w<<endl;  int ww; cin>>ww;}
   deltaA= fabs(pow(10.0,-0.4*errw[w])-1.0) * Astar0;//deltaA= deltaF/F_0
   tt= RandN(deltaA,3.0);
   Astar=double(Astar0 + tt);///magnification factor
   magni= s.magb[4]-2.5*log10(Astar);//observed magnitude
   chi0 +=fabs( (magni-s.magb[4])*(magni-s.magb[4])/(errw[w]*errw[w]));
   chi1 +=fabs( (magni- magni0  )*(magni- magni0  )/(errw[w]*errw[w]));
  // if(flagf>0) fprintf(data2,"%.5lf   %.5lf    %.8lf   %.5lf  %.5lf  %.5lf\n",ptt*24.0, Astar, deltaA, magni , errw[w], tim);
   t_1= tim; 
   if( double((t_1-l.t0)*(t_0-l.t0))<=0.0 and nd>0 and fabs(t_0-t_1)<3.0*cade) flag_peak=1;
   t_0=t_1; 
   if(fabs(magni-s.magb[4])>fabs(3.0*errw[w])) flag[nd]=1;
   else                                        flag[nd]=0;
   if(nd>4 and float(flag[nd]*1.0+flag[nd-1]*1.0+flag[nd-2]+flag[nd-3]+flag[nd-4]+flag[nd-5]+0.0)>5.5)  flag_det=1;
   nd+=1; }
   if(timp>cade or timp==cade)  timp=double(timp-cade); 
   }///for in time
  // if(flagf>0) {fclose(data2); fclose(magg);}
   dchi= fabs(chi1-chi0);
   if(flag_peak>0 and dchi>300.0  and flag_det>0 and nd>5){Nml+=1;   flag_ml=1;} 
   else flag_ml=0;
   /*if(flag_ml<1){
   fil1=fopen("./files/pmls_1.txt","a+");
   fil2=fopen("./files/pmls_2.txt","a+");
   fprintf(fil1,"%d  %.3lf  %.3lf  %d  %d  %e  %e  %e  %.5lf  %.4lf  %.4lf  %d  %.5lf  %d  %d  %.3lf  %.4lf  %.5lf  %.4lf  %.4lf %.5lf  %.1lf  %.5lf  %.4lf   %.4lf  %.2lf  %.4lf  %.4lf  %.4lf  %.3lf %.3lf %d  %.10lf  %.10lf  %.4lf  %.4lf  %.6lf %.3lf %d %d %.1lf %d %d %d %.3lf %.2lf  %.1lf  %.1lf\n",
   Nsi, s.bg, s.lg,
   l.struc, l.numl, log10(l.Ml), l.Mlj, double(l.Rl/Rsun), log10(l.ro_lens), l.Dl, l.vl, l.type,l.tstar,  ///13
   ...
   log10(l.tE), log10(l.RE/AU), l.mul, l.Vt, log10(fabs(s.u0)), l.t0, //38
   flag_peak,flag_det, dchi, flag_ml, Nml, nd, s.SNR[4],fl.finiteL, Nmw, Nsmc);//48
   for(int i=0;i<M; ++i)
   fprintf(fil2,"%d  %.3lf  %.3lf  %.4lf  %.3lf   %.4lf   %.1lf  %.3lf\n",
   Nsi, s.Mab[i], s.Map[i], s.Ai[i], s.magb[i], s.blend[i], s.Nblend[i], s.SNR[i]); 
   fclose(fil1);  fclose(fil2); }*/
   delete [] flag;  
   
///====================================================================================================== 
   d.oplb1[fla]+=double(s.opt*1.0e6) *wei;
   d.nsim[fla]  +=1.0 *wei;  nnsi[fla]+=1.0;
   if(flag_ml>0){
   bbv=-1.0; 
   if(l.tE<lte[0]){           
   slope= 26.60;
   bbv= pow(10.0, double(log10(eff[0])+slope*(log10(l.tE) - log10(lte[0]))) );}
   else if(l.tE>lte[nte-1] or l.tE==lte[nte-1] ){
   slope=-0.29; 
   bbv= pow(10.0, double(log10(eff[nte-1])+slope*(log10(l.tE) - log10(lte[nte-1])) ) );} 
   else{
   for(int i=0;i<int(nte-1); ++i){
   if((l.tE-lte[i])*(l.tE-lte[i+1])<0.0 or l.tE==lte[i]){bbv=eff[i];  break;}}}
   if(bbv<0.0 or bbv>1.0){cout<<"ERROR:  l.tE: "<<l.tE<<"\t effi:  "<<bbv<<endl;  int yye;  cin>>yye;}
   
   d.ndet[fla] +=1.0*wei;  nnde[fla]+=1.0;
   d.efii[fla]+=bbv*wei; 
   d.tE[fla] +=l.tE*wei;
   d.ML[fla] +=l.Ml*wei;
   d.RE[fla] +=double(l.RE/AU)*wei;
   d.rho[fla]+=s.ro_star*wei;
   d.Dl[fla] +=l.Dl*wei; 
   d.Ds[fla] +=s.Ds*wei; 
   d.rhol[fla]+=l.ro_lens*wei;
   d.oplb2[fla] +=double(s.opt*1.0e6)*wei;
   d.efte[fla]  +=double(bbv/l.tE)*wei;
   
   if(int(d.ndet[0])%10==0){
   cout<<"============================================================="<<endl;
   cout<<"No. sim :"<<Nsi<<"\t No. *det: "<<Nst<<"\t No. ML:  "<<Nml<<endl; 
   cout<<"l1:  "<<l1a<<"\t l2:  "<<l2a<<endl;
   ...
   cout<<"flag_det:  "<<flag_det<<"\t flag_peak:  "<<flag_peak<<"\t dchi:  "<<dchi<<endl; 
   cout<<"flag_ml:  "<<flag_ml<<"\t nd:  "<<nd<<endl;
   cout<<"Nmw:  "<<Nmw<<"\t Nsmc:  "<<Nsmc<<endl;
   cout<<"==============================================================="<<endl;}
   cout<<"ndet[0]:  "<<d.ndet[0]<<"\t ndet[1]:    "<<d.ndet[1]<<endl;}}
   Nsi+=1;
   }while( double(Nml*1.0)<double(700.0+ mstep*4.0) ); 
///============================================================

   nnst=42547600.000 ;//605504.0; 
   opp =0.148407 ;//0.0352996; 
   for(int i=1; i<(Num-2); i+=1){
   effe=double(Nstlb[i]/(Nsilb[i]+ epss));
   s.nstart +=(s.Rostari[i])*(s.Nstart/s.Rostart)*effe;}///Nstar/deg^{2}
 

    nsi= double(d.nsim[0] + d.nsim[1]); 
    nde= double(d.ndet[0] + d.ndet[1]); 
    te=  double(d.tE[0] + d.tE[1]); 
    ml=  double(d.ML[0] + d.ML[1]); 
    re=  double(d.RE[0] + d.RE[1]);
    rho= double(d.rho[0]+d.rho[1]); 
    dl=  double(d.Dl[0] + d.Dl[1]); 
    ds=  double(d.Ds[0] +  d.Ds[1]); 
    rhol=double(d.rhol[0]+d.rhol[1]); 
    efte=double(d.efte[0]+ d.efte[1]); 
    op1= double(d.oplb1[0]+d.oplb1[1]);
    op2= double(d.oplb2[0]+d.oplb2[1]);
    Efii= double(d.efii[0]+d.efii[1]);
    EF= double(nnde[0]+nnde[1])*100.0/(nnsi[0]+nnsi[1]) ;///double(nde*100.0/nsi);  
    te= double(te/nde);    re=double(re/nde); ml= double(ml/nde);
    rho=double(rho/nde);   dl=double(dl/nde); 
    ds= double(ds/nde);    rhol=double(rhol/nde); 
    efte=double(efte/nde); op1=double(op1/nsi);    Efii=double(Efii/nde); 
    op2= double(op2/nde); 
    gam= double( 2.0/M_PI * efte * op2 *1.0e-6 *season ); 
    Nev2= double(gam * nnst  ); 
    ml=  double(ml*Msun/Mearth);  
    avenn+= double (s.nstart);
    aveop+= double(op1); 
   
    fil5=fopen("./files/tab5_paper.txt","a+");
    fil6=fopen("./files/tab5_paperb.txt","a+"); 
    for(int j=0; j<2;   ++j){
    d.Ep[j]=double(nnde[j]*100.0/nnsi[j]) ; // double(d.ndet[j]*100.0/d.nsim[j]);
    d.oplb1[j]= double(d.oplb1[j]/d.nsim[j]); 
    d.tE[j]=   double( d.tE[j]/ d.ndet[j]);
    d.ML[j]=   double( d.ML[j]/ d.ndet[j]);
    d.efii[j]= double(d.efii[j]/d.ndet[j]);
    d.RE[j]=   double( d.RE[j]/ d.ndet[j]);
    d.rho[j]=  double(d.rho[j]/ d.ndet[j]);
    d.Dl[j]=   double( d.Dl[j]/ d.ndet[j]);
    d.Ds[j]=   double( d.Ds[j]/ d.ndet[j]);
    d.rhol[j]= double(d.rhol[j]/d.ndet[j]);
    d.efte[j]= double(d.efte[j]/ d.ndet[j]); 
    d.oplb2[j]=double(d.oplb2[j]/d.ndet[j]); 
    d.gama[j]= double(2.0/M_PI * d.efte[j] * d.oplb2[j] *1.0e-6 *season);////star^{-1}season^{-1}
    //d.Nevent[j]=d.gama[j] * nnst; ///Ne/deg^{2}/season
    d.Nevent2[j]=d.gama[j] * nnst ; ///Ne/deg^{2}/season
    d.ML[j]= double(d.ML[j]*Msun/Mearth);


    cout<<"step:  "<<j<<"<Ml>:    "<<d.ML[j]<<endl;
    cout<<"gama:  "<<d.gama[j]<<"\t Nevent:  "<<d.Nevent[j]<<"\t effi:  "<<d.Ep[j]<<endl; 
    fprintf(fil6,"%.1lf  %.1lf  %.7lf  %.7lf  %.7lf %.7lf  %.5lf  %.5lf  %.7lf  %.8lf   %.7lf  %.7lf  %.7lf  %.5lf %.5lf %.5lf %.5lf\n",
    d.nsim[j], d.ndet[j], log10(d.tE[j]), log10(d.ML[j]), log10(d.RE[j]), log10(d.rho[j]), d.Dl[j], d.Ds[j], log10(d.rhol[j]), 
    d.efte[j], d.oplb1[j], d.oplb2[j], d.Ep[j], d.gama[j]*1000000.0, d.Nevent[j], d.efii[j], d.Nevent2[j]); 
    
    
    
    fprintf(fil5,"$rm{I}$ & $%.1lf$ & $%.2lf$ & $%.3lf$ & $%.2lf$ & $%.2lf$ & $%.2lf$ & $%.2lf$ & $%.1lf$ & $%.1lf$\n",
    log10(d.ML[j]), log10(d.RE[j]), d.tE[j], log10(d.rho[j]), log10(d.rhol[j]), d.gama[j]*1000000.0, d.oplb2[j]*10.0,d.Nevent2[j],d.Ep[j]);}
    
    fprintf(fil5,"$rm{Total}$ & $%.1lf$ & $%.2lf$ & $%.3lf$ & $%.2lf$ & $%.2lf$ & $%.2lf$ & $%.2lf$ & $%.1lf$ & $%.1lf$\n",
    log10(ml), log10(re),  te , log10(rho), log10(rhol), gam*1000000.0, op2*10.0, Nev2, EF);  
   
  
  
  
    fprintf(fil6,"%.1lf  %.1lf  %.7lf  %.7lf  %.7lf %.7lf  %.5lf  %.5lf  %.7lf  %.8lf   %.7lf  %.7lf  %.7lf %.5lf  %.5lf %.5lf %.5lf\n",
    nsi, nde, log10(te), log10(ml), log10(re), log10(rho), dl, ds, log10(rhol), efte, op1, op2, EF, gam*1000000.0, Nev, Efii, Nev2); 
    ...
    cout<<"\n  ml:  "<<ml<<endl;
    cout<<"Nstar:  "<<s.nstart<<"\t tau_StarD: "<<op1<<"\t tau_ML: "<<op2<<endl;
    cout<<"Overal:  "<<"\t gama:  "<<gam<<"\t Nevent:  "<<Nev<<"\t effi:  "<<EF<<endl;
    cout<<"Nev:  "<<Nev<<"\t Nev2:   "<<Nev2<<endl;
    cout<<"HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH"<<endl; }
    cout<<"<nstar:  >  %.10lf: "<<double(avenn/mtss)<<"\t <opt>:  %.10lf  "<<double(aveop/mtss)<<endl;
    return(0);
}
////==========================================================================
///==============================================================//
...
void optical_depth(source & s)
{
    ...
    s.od_dsmc +=  s.rho_dsmc[k]*x*(1.0-x)*dx*CC;
    s.od_hsmc +=  s.rho_hsmc[k]*x*(1.0-x)*dx*CC;  }
    s.opt= fabs(s.od_disk+s.od_ThD+s.od_bulge+s.od_halo+ s.od_dsmc + s.od_hsmc );///total
...
else if (rf<=(s.rho_disk[nums]+s.rho_bulge[nums]+s.rho_ThD[nums]+ s.rho_halo[nums]+s.rho_hsmc[nums]))struc=4;///halo_smc
else if (rf<=(s.rho_disk[nums]+s.rho_bulge[nums]+s.rho_ThD[nums]+ s.rho_halo[nums]+s.rho_hsmc[nums]+s.rho_dsmc[nums]))struc=5;//disk_smc
...
void func_lens(lens & l, source & s, int mstep)
{
   ...
   double  randflag=((double)rand()/(double)(RAND_MAX+1.))*s.Rostar0[l.numl];
  ...
  else if (randflag<=fabs( s.Rostar0[l.numl]-s.rho_dsmc[l.numl]) ) l.struc=4;//halo_smc
  else if (randflag<=fabs( s.Rostar0[l.numl]) )                     l.struc=5;//disk_smc
  else {  cout<<"randflag: "<<randflag<<"\t rho_star0: "<<s.Rostar0[l.numl]<<endl;  int ye; cin>>ye;}


    double min= pow(10.0,double(-3.0+mstep*0.1));
    l.Ml= double(min + RandN(min*0.02,4.0) )*Mearth/Msun;
    ...
    l.t0=72.0*(double)rand()/(double)(RAND_MAX+1.); 
    l.pt1= -3.5*l.tE + l.t0; 
    l.pt2= +3.5*l.tE + l.t0; 
    ...
    cout<<"BIG ERROR te: "<<l.tE<<"\t RE(AU): "<<l.RE/AU<<"\t V_rel: "<<l.Vt<<endl;
    int iie; cin>>iie;}
...
void Disk_model(source & s, int l1, int l2)
{
   ...
   double pos1,pos2, inc1, inc2, sx, sy, sz, rho1, rho2;
   double rho0h, turn, alm, r0; 
   double x0, y0, z0; 
   double xol, yol, zol; 
...
///=====================  SMC density profiles ======================
 
   x0 = -x*cos(s.decli/RA)*sin(s.right/RA-RaSMC/RA);
   y0 =  x*sin(s.decli/RA)*cos(DecSMC/RA)-x*cos(s.decli/RA)*sin(DecSMC/RA)*cos(s.right/RA-RaSMC/RA);
   z0 = DSMC- x*cos(s.decli/RA)*cos(DecSMC/RA)*cos(s.right/RA-RaSMC/RA)-x*sin(s.decli/RA)*sin(DecSMC/RA);
   if(fabs(x-DSMC)<double(step*1.5)){s.xv=x0;  s.yv=y0;   s.zv=z0;}
...
///=====================  SMC density HALO ======================
   alm=1.5;
   turn=8.5;
   rho0h=2.4/(4.0*M_PI*alm*alm*(turn-alm*atan(turn/alm))); 
   if(r0<turn)  s.rho_hsmc[i]=fabs(rho0h*frac/(1.0+r0*r0/alm/alm))*0.7;  ///M_sun/PC^3
   else         s.rho_hsmc[i]=0.0;   
   
///==================================================================
   pos1=(90.0-66.0)*M_PI/180.0; 
   inc1=74.0*M_PI/180.0;  
   sx=0.8 , sy=3.5, sz=1.3;
   ...
   rho1= 8.5*0.001*exp(-0.5*(xol*xol/sx/sx + yol*yol/sy/sy + zol*zol/sz/sz)); 
   ///******************************************************
   pos2=(90.0-83.00)*M_PI/180.0; 
   inc2=0.0;
   sx=0.8;  sy=1.2;  sz=2.1; 
   xol= x0*cos(pos2) +  y0*sin(pos2);
   yol=-x0*sin(pos2)*cos(inc2) + y0*cos(pos2)*cos(inc2) -z0*sin(inc2);
   zol=-x0*sin(pos2)*sin(inc2) + y0*cos(pos2)*sin(inc2) +z0*cos(inc2); 
   rho2= 3.9*0.01*exp(-sqrt(xol*xol/sx/sx + yol*yol/sy/sy))*exp(-0.5*zol*zol/sz/sz); 
   s.rho_dsmc[i] = double(rho1+ rho2)*0.7;  
...
s.Rostar0[i]=fabs(s.rho_disk[i])+fabs(s.rho_ThD[i])+fabs(s.rho_bulge[i])+fabs(s.rho_halo[i]) +fabs(s.rho_hsmc[i])+fabs(s.rho_dsmc[i]);///[M_sun/pc^3]

s.Rostari[i]=s.Rostar0[i]*x*x*step*1.0e9*(M_PI/180.0)*(M_PI/180.0);///[M_sun/deg^2]

s.Nstari[i]= binary_fraction*((s.rho_disk[i]+s.rho_dsmc[i])*fd/0.403445+s.rho_ThD[i]*fh/0.4542+(s.rho_halo[i]+s.rho_hsmc[i])*fh/0.4542+(s.rho_bulge[i])*fb/0.308571);////[Nt/pc^3] 
...
//if(flagf>0)
//fprintf(fill,"%e   %e   %e   %e   %e  %e  %e  %e   %e\n",
///  x,s.rho_disk[i],s.rho_bulge[i],s.rho_ThD[i],s.rho_halo[i],s.rho_dsmc[i],s.rho_hsmc[i], s.Rostar0[i],s.Nstari[i]); 
 // cout<<"rho_disk(SMC):  "<<s.rho_dsmc[i]<<endl;
  //cout<<"rho_halo(SMC):  "<<s.rho_hsmc[i]<<endl;
   }
...
//if(flagf>0)  fclose(fill);
   ...
   //cout<<"distance_SMC_Center:  "<<rlm<<"\t projected_distance:  "<<Rlm<<endl;
...
if (l.Dl==0.0) l.Dl=0.00034735;
 ...
 for(int i=0; i<8; ++i){  Rho[i]=rho0[i]*corr[i]/d0[i]; maxr=maxr+ Rho[i];}
  ...
  double v_R_smc=+19.0;
  double v_T_smc=-153.0; 
  double v_Z_smc= 153.0;
  double sigma_SMC=27.5; 
  double err_rsmc= 18.0; ///error of global velocity
  double err_tsmc= 21.0; 
  double err_zsmc= 17.0; 
...
/// Generate Source velocity components in Glactocenteric cylindrical coordinates(x',y')
   ...
   SVR= RandN(sigma_SMC, NN); 
   SVT= RandN(sigma_SMC, NN); 
   SVZ= RandN(sigma_SMC, NN); 
   SVZ +=  v_Z_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_zsmc;
   SVR +=  v_R_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_rsmc;
   SVT +=  v_T_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_tsmc;}
...
/// Generate Lens velocity components in Glactocenteric cylindrical coordinates(x',y'
   ...
   LVR= RandN(sigma_SMC, NN); 
   LVT= RandN(sigma_SMC, NN); 
   LVZ= RandN(sigma_SMC, NN); 
   LVZ +=  v_Z_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_zsmc;
   LVR +=  v_R_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_rsmc;
   LVT +=  v_T_smc + ((double)rand()/(double)(RAND_MAX+1.)*2.0-1.0)*err_tsmc;}
