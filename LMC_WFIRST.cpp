...
const int nte=70;/// No. rows of JohnSon_Fig6_sort.txt 
const int nrd=10000; 
...
struct dete{
    ...
    double oplb1[2], oplb2[2], gama[2], Nevent[2], Nevent2[3];    
    double efte[2]; 
    double ML[2]; 
    double efii[2]; 
};
...
void func_lens(lens & l, source & s, int);
...
///****************************************************************************
   ...
   cout<<"*********************"<<"\t ro_max:  "<<s.Romax<<endl;
    ...
    int flag_det, flag_peak, Nmicro, flag_ml, nd, Nml=0, Nsi=0, w, icon;
...
///===========================================================  
    ...
    FILE* effitE; 
    effitE=fopen("./files/effiLMC.dat","r");
    for(int i=0; i<nte;++i){
    fscanf(effitE,"%lf     %lf\n",&lte[i],&eff[i]);
    if(lte[i]<0.0 or eff[i]>1.0008675340 or eff[i]<1.0e-9  or (i>0  and lte[i]<lte[i-1]) ){
    cout<<"ERROR lte[i]:  "<<lte[i]<<"\t eff:  "<<eff[i]<<endl; int iis;  cin>>iis;}}
    fclose(effitE);   
    cout<<" effiLMC.txt  was read !!!  "<<endl;



   
///=========================================================== 
    FILE* fil1;   FILE* fil2;   FILE* fil6; 
    FILE* fil5;   
    vbb.LoadESPLTable("./files/ESPL.tbl");
   // fil1=fopen("./files/par_1.txt","w");
   // fil2=fopen("./files/par_2.txt","w");
    fil5=fopen("./files/tab4_paper.txt","w");
    fclose(fil5); 
     fil6=fopen("./files/tab4_paperb.txt","w");
    fclose(fil6); 
    //fclose(fil1); 
    //fclose(fil2); 
    
   int l1a, l2a;  
   double te, re, rho, dl, ds, rhol, efte, op1, op2, EF, gam, Nev, Nev2, ml, Efii, nsi, nde, flagb, opp, nsst ; 
   double avenn=0.0, aveop=0.0;
   double Nmw=0.0, Nlmc=0.0; 
   double mstt=0.0;
   double dec1= -72.5; 
   double dec2= -64.8; 
   double rig1=float( (4.0+27.0/60.0)*15.0 );
   double rig2=float( (5.0+50.0/60.0)*15.0 );
   double ddec= fabs(dec2-dec1)/nn/1.0;
   double drig= fabs(rig2-rig1)/nn/1.0;
   cout<<"ddec:  "<<ddec<<"\t drig:   "<<drig<<endl;
   double exti[455]={0.0}; 
   ...
   cout<<"*********************** mstep:   "<<mstep<<endl;
    ...
    s.nstart=0.0; 
    Nmw=0.0;  Nlmc=0.0;  Nsi=0;

    

//===================== Monte Carlo Simulation ================================
   do{
   s.right= (double)rand()/(double)(RAND_MAX+1.)*1.4 + RaLMC;
   s.decli= (double)rand()/(double)(RAND_MAX+1.)*1.4 + DecLMC;
...
///==================================================================================================
   ...
   deltaA= fabs(pow(10.0,-0.4*errw[w])-1.0) * Astar0 ;
   tt= RandN(deltaA,3.0);
   ...
   nd+=1; }
   if(timp>cade or timp==cade)  timp=double(timp-cade);   }///for in time
   if(flagf>0) {fclose(data2); fclose(magg);}
   dchi= fabs(chi1-chi0);
   if(flag_peak>0 and dchi>300.0  and flag_det>0 and nd>5){Nml+=1;   flag_ml=1;} 
   else flag_ml=0;  
 /*if(flag_ml<1){
   fil1=fopen("./files/par_1.txt","a+");
   fil2=fopen("./files/par_2.txt","a+");
   ...
   flag_peak,flag_det, dchi, flag_ml, Nml, nd, s.SNR[4],fl.finiteL, Nmw, Nlmc);//48
   for(int i=0;i<M; ++i)
...
///======================================================================================================   
   ...
   cout<<"============================================================="<<endl;
   cout<<"No. sim :"<<Nsi<<"\t No. ML:  "<<Nml<<endl; 
   cout<<"l1:  "<<l1a<<"\t l2:  "<<l2a<<"\t icon:  "<<icon<<endl;
   ...
   cout<<"Nmw:  "<<Nmw<<"\t Nlmc:  "<<Nlmc<<endl;
   cout<<"==============================================================="<<endl;} 
   cout<<"ndet[0]:  "<<d.ndet[0]<<"\t ndet[1]:  "<<d.ndet[1]<<endl;}}
   }while( double(Nml*1.0)<double(1500.0+ mstep*9.0) ); 
///============================================================

   opp=0.0681433 ;// 0.0542051; 
   nsst=144616000.0 ;//9.93816*1000000.0;
   ...
   s.nstart += s.Rostari[i]*(s.Nstart/s.Rostart)*effe;}///Nstar/deg^{2}
    ...
    Nev2= double(gam * nsst ); 
    ml= double(ml*Msun/Mearth);
    avenn+= s.nstart;
    aveop+= op1;  
  
        
   fil5=fopen("./files/tab4_paper.txt","a+"); 
   fil6=fopen("./files/tab4_paperb.txt","a+"); 
   for(int j=0; j<2;   ++j){
  ...
  // d.Nevent[j]= d.gama[j] * nsst; /// Ne/deg^{2}/season
   d.Nevent2[j]= d.gama[j] * nsst;// * /opp; /// Ne/deg^{2}/season
   d.ML[j]= double(d.ML[j]*Msun/Mearth);
   cout<<"<Ml>:    "<<d.ML[j]<<"\t structure:  "<<j<<endl;
   cout<<"step:  "<<j<<"\t gama:  "<<d.gama[j]<<"\t Nevent:  "<<d.Nevent[j]<<"\t effi:  "<<d.Ep[j]<<endl;  
   fprintf(fil6,"%.1lf  %.1lf  %.7lf  %.7lf  %.7lf   %.7lf  %.5lf  %.5lf  %.7lf  %.8lf %.7lf  %.7lf  %.7lf   %.5lf  %.5lf %.5lf %.5lf\n",
   d.nsim[j], d.ndet[j], log10(d.tE[j]), log10(d.ML[j]), log10(d.RE[j]), log10(d.rho[j]), d.Dl[j], d.Ds[j], log10(d.rhol[j]), d.efte[j], d.oplb1[j], d.oplb2[j], d.Ep[j], d.gama[j]*1000000.0, d.Nevent[j], d.efii[j] , d.Nevent2[j] ); 
...
////******************************************************************************
   ...
   fprintf(fil6,"%.1lf  %.1lf  %.7lf  %.7lf  %.7lf   %.7lf  %.5lf  %.5lf  %.7lf  %.7lf  %.7lf  %.7lf  %.7lf %.5lf  %.5lf %.5lf %.5lf\n",
   ...
   cout<<"<nstar:  >  %.10lf:      "<<double(avenn/mstt)<<"\t <opt>:    %.10lf  "<<double(aveop/mstt)<<endl;
   return(0);
}
////==========================================================================
///==============================================================//
...
///HHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
   ...
   ///Av=2.3 - (1.0-(s.Rostart/s.Romax) )*2.0;
   Av=s.Avv;
